...
def test_app(mock_agent):
    ...
    with patch("agents.sync_agent.api.sync.get_agent", return_value=mock_agent), \
         patch("agents.sync_agent.app.main.agent", mock_agent), \
         patch("agents.sync_agent.app.main.settings") as mock_settings:
        ...
        from agents.sync_agent.api.sync import router as sync_router
...
async def test_list_mappings(test_app, db_session):
    ...
    with patch("agents.sync_agent.api.sync.get_db") as mock_get_db:
        async def _override():
            yield db_session
