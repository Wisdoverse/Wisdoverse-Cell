"""DevOutboxDispatcherPlugin — background Dev outbox retry loop."""
...
logger = get_logger("plugin.dev-outbox-dispatcher")

_RUNTIME_LABEL = "dev-agent"


class DevOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending Dev integration events."""

    name = "dev-outbox-dispatcher"
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("dev_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_dev_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("dev_outbox_dispatcher_missing")
            return
