...
class TestPJMOutboxDispatcherPlugin:
    @pytest.mark.asyncio
    async def test_dispatch_once_calls_agent_outbox_dispatcher(self):
        from agents.pjm_agent.app.plugins.outbox_dispatcher import (
            PJMOutboxDispatcherPlugin,
        )

        plugin = PJMOutboxDispatcherPlugin(interval=100, batch_size=7)
        runtime = MagicMock()
        runtime.agent.publish_pending_pjm_events = AsyncMock(
            return_value={"total": 1, "published": 1, "failed": 0}
        )

        await plugin._dispatch_once(runtime)

        runtime.agent.publish_pending_pjm_events.assert_awaited_once_with(limit=7)
    ...
    async def test_health_ok_when_running(self):
        ...
        plugin = PJMOutboxDispatcherPlugin(interval=100)
