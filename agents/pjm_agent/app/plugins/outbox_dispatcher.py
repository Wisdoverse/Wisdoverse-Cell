"""PJMOutboxDispatcherPlugin — background PJM outbox retry loop."""
...
logger = get_logger("plugin.pjm-outbox-dispatcher")

_RUNTIME_LABEL = "pjm-agent"


class PJMOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending PJM integration events."""

    name = "pjm-outbox-dispatcher"
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("pjm_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_pjm_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("pjm_outbox_dispatcher_missing")
            return
