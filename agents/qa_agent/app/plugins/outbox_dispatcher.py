"""QAOutboxDispatcherPlugin — background QA outbox retry loop."""
...
logger = get_logger("plugin.qa-outbox-dispatcher")

_RUNTIME_LABEL = "qa-agent"


class QAOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending QA integration events."""

    name = "qa-outbox-dispatcher"
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("qa_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_qa_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("qa_outbox_dispatcher_missing")
            return
