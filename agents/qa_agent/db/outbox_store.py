"""SQLAlchemy adapter for QA event outbox persistence."""
...
class SqlAlchemyQAEventOutboxStore(QAEventOutboxStore):
    """SQLAlchemy-backed QA event outbox store."""
    ...
    async def stage(self, session: object, event: Event) -> None:
        outbox = QAEventOutboxRepository(session)
