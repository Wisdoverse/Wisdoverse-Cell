...
def mock_redis():
    ...
    mock_r = AsyncMock()
    with patch("agents.gateways.user_interaction.api.webhook._get_redis", return_value=mock_r):
        yield mock_r
...
async def test_is_duplicate_first_time(mock_redis):
    ...
    from agents.gateways.user_interaction.api.webhook import _is_duplicate
...
def test_challenge_response():
    ...
    from agents.gateways.user_interaction.api.schemas import ChallengeResponse
