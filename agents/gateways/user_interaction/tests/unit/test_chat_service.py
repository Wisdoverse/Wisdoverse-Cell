...
@pytest.fixture
def chat_svc():
    with patch("agents.gateways.user_interaction.core.chat_service.llm_gateway") as mock_gw:
        with patch("agents.gateways.user_interaction.core.chat_service.settings") as mock_settings:
...
async def test_chat_tool_loop_terminates_at_max(chat_svc):
    ...
    with patch(
        "agents.gateways.user_interaction.core.chat_service.ToolExecutor.execute",
        new_callable=AsyncMock,
        return_value='{"result": "ok"}',
    ):
        from agents.gateways.user_interaction.core.chat_service import MAX_TOOL_CALLS
        await chat_svc.chat("hello", user_id="u1")
...
async def test_model_uses_chat_model_not_default(chat_svc):
    ...
    with patch("agents.gateways.user_interaction.core.chat_service.settings") as mock_s:
...
async def test_chat_with_user_assistant_no_open_id_in_prompt(chat_svc):
    ...
    with patch("agents.gateways.user_interaction.core.chat_service.db_manager") as mock_db:
        mock_session = AsyncMock()
        mock_db.session.return_value.__aenter__ = AsyncMock(return_value=mock_session)
        mock_db.session.return_value.__aexit__ = AsyncMock(return_value=False)

        with patch(
            "agents.gateways.user_interaction.core.chat_service.DailyProgressRepository",
        ) as MockRepo:
