...
from agents.gateways.user_interaction.api.bitable import (
    ConfirmRequest,
    CreateRequest,
    _sanitize_fields,
    confirm_update,
...
async def test_confirm_update_returns_error_card_on_failure():
    ...
    with patch("agents.gateways.user_interaction.api.bitable.bitable_service") as mock_svc, \
         patch("agents.gateways.user_interaction.api.bitable._resolve_duplex_links", new_callable=AsyncMock, side_effect=lambda f, **kw: f), \
         patch("agents.gateways.user_interaction.api.bitable.record_op", new_callable=AsyncMock):
