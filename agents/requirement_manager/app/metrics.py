...
REQUIREMENTS_EXTRACTED = Counter(
    "wisdoverse-cell_requirements_extracted_total",
    "Total requirements extracted",
    ["source"],
)

REQUIREMENTS_CONFIRMED = Counter(
    "wisdoverse-cell_requirements_confirmed_total",
    "Total requirements confirmed by humans",
)

# ── LLM ───────────────────────────────────────────────────────────────────
LLM_REQUESTS = Counter(
    "wisdoverse-cell_llm_requests_total",
    "Total LLM API requests",
    ["model", "task_type", "success"],
)

LLM_LATENCY = Histogram(
    "wisdoverse-cell_llm_latency_seconds",
...
DB_POOL_CHECKED_OUT = Gauge(
    "wisdoverse-cell_db_pool_checked_out",
    "Number of currently checked-out DB connections",
)

# ── Events ────────────────────────────────────────────────────────────────
EVENTS_PUBLISHED = Counter(
    "wisdoverse-cell_events_published_total",
