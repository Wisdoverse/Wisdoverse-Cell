...
@pytest.fixture
def chat_svc():
    with patch("agents.chat_agent.core.chat_service.llm_gateway") as mock_gw:
        with patch("agents.chat_agent.core.chat_service.settings") as mock_settings:
...
async def test_model_uses_chat_model_not_default(chat_svc):
    ...
    with patch("agents.chat_agent.core.chat_service.settings") as mock_s:
...
# ---------------------------------------------------------------------------
# chat_with_pm_persona SEC-003: open_id must NOT appear in system prompt
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_chat_with_pm_persona_no_open_id_in_prompt(chat_svc):
