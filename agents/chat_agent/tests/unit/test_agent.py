...
@pytest.mark.asyncio
async def test_handle_request_chat_pm(mock_event_bus, mock_chat_service):
    """验证 handle_request action=chat_pm 调用 PM 人设聊天"""
    from agents.chat_agent.service.agent import ChatAgent
    ...
    assert result == {"reply": "mock reply"}
    mock_chat_service.chat_with_pm_persona.assert_awaited_once_with(
        message="项目进度如何",
        user_id="user2",
        user_name="张三",
        context={
