...
def _patch_db():
    ...
    with patch(
        "agents.chat_agent.core.chat_service.db_manager",
        new=MagicMock(),
    ) as mock_db:
        ...
        with patch(
            "agents.chat_agent.core.chat_service.ConversationRepository",
        ) as mock_repo_cls:
            repo_inst = AsyncMock()
...
class TestPMPersonaPromptToolSearch:
    """PM_PERSONA_PROMPT should include tool_search guidance."""

    def test_prompt_mentions_tool_search(self):
        from agents.chat_agent.core.chat_service import PM_PERSONA_PROMPT
        assert "tool_search" in PM_PERSONA_PROMPT

    def test_prompt_teaches_search_strategy_without_tool_inventory(self):
        from agents.chat_agent.core.chat_service import PM_PERSONA_PROMPT
        assert "不熟悉的工具用 tool_search 搜索" in PM_PERSONA_PROMPT
        assert "sync_now" not in PM_PERSONA_PROMPT
        assert "add_bitable_field" not in PM_PERSONA_PROMPT
