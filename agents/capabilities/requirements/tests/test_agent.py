...
from agents.capabilities.requirements.service.agent import (
    IngestResult,
    RequirementManagerAgent,
    agent,
)
from agents.capabilities.requirements.service.event_handlers import dispatch_event
from shared.integrations import feishu as _feishu_mod
from shared.integrations.feishu.cards import requirement as _req_cards_mod
...
class TestEventDispatch:
    ...
    async def test_dispatch_logs_unhandled_event(self):
        ...
        with patch("agents.capabilities.requirements.service.event_handlers.logger") as mock_logger:
...
class TestAgentDependencyInjection:
    ...
    def test_agent_uses_defaults_when_no_injection(self):
        ...
        from agents.capabilities.requirements.db.vector_store import vector_store
...
class TestExtractFromSession:
    ...
    async def test_extract_from_session_no_messages(self):
        ...
        with patch("agents.capabilities.requirements.service.agent.MessageRepository") as MockMsgRepo:
    ...
    async def test_extract_from_session_with_messages(self):
        ...
        with (
            patch("agents.capabilities.requirements.service.agent.MessageRepository") as MockMsgRepo,
            patch("agents.capabilities.requirements.service.agent.RequirementRepository") as MockReqRepo,
    ...
    async def test_extract_from_session_no_requirements_extracted(self):
        ...
        with (
            patch("agents.capabilities.requirements.service.agent.MessageRepository") as MockMsgRepo,
            patch("agents.capabilities.requirements.service.agent.RequirementRepository"),
            patch.object(test_agent, "ingest_meeting", new_callable=AsyncMock) as mock_ingest,
            patch.object(
                test_agent, "_send_session_extraction_card", new_callable=AsyncMock
            ) as mock_send_card,
