...
from agents.capabilities.requirements.service.agent import IngestResult
...
def mock_agent():
    ...
    with patch("agents.capabilities.requirements.api.ingest.get_agent", return_value=mock_agent_instance), \
         patch("agents.capabilities.requirements.api.feedback.get_agent", return_value=mock_agent_instance):
...
def mock_db_session():
    """创建 mock 数据库会话"""
    with patch("agents.capabilities.requirements.api.ingest.get_db") as mock_get_db, \
         patch("agents.capabilities.requirements.api.feedback.get_db") as mock_get_db_feedback:
...
class TestIngestAPI:
    ...
    async def test_upload_delegates_to_agent(self, mock_agent, mock_db_session):
        ...
        with patch("agents.capabilities.requirements.app.main.agent") as mock_main_agent:
...
class TestAgentEventPublishing:
    ...
    async def test_confirm_publishes_event(self):
        ...
        with patch("agents.capabilities.requirements.service.agent.RequirementRepository") as MockRepo:
