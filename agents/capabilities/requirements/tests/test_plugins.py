...
class TestGrpcPlugin:
    ...
    async def test_startup_calls_factory(self):
        from agents.capabilities.requirements.app.plugins.grpc import GrpcPlugin
...
class TestSessionTimeoutPlugin:
    ...
    async def test_skips_when_disabled(self):
        from agents.capabilities.requirements.app.plugins.session_timeout import (
            SessionTimeoutPlugin,
        )

        with patch("agents.capabilities.requirements.app.plugins.session_timeout.settings") as s:
    ...
    async def test_health_ok_when_running(self):
        ...
        with (
            patch("agents.capabilities.requirements.app.plugins.session_timeout.settings") as s,
            patch(
                "agents.capabilities.requirements.app.plugins.session_timeout.get_session_manager"
            ) as gsm,
        ):
...
class TestFeishuGatewayPlugin:
    ...
    async def test_skips_when_feishu_disabled(self):
        from agents.capabilities.requirements.app.plugins.feishu_gateway import FeishuGatewayPlugin

        with patch("agents.capabilities.requirements.app.plugins.feishu_gateway.settings") as s:
...
class TestChannelRegistryPlugin:
    ...
    async def test_no_channels_returns_empty(self):
        from agents.capabilities.requirements.app.plugins.channel_registry import (
            ChannelRegistryPlugin,
        )

        with patch("agents.capabilities.requirements.app.plugins.channel_registry.settings") as s:
            s.feishu_enabled = False
