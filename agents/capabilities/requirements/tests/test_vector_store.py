...
from agents.capabilities.requirements.db.vector_store import VectorStore, _milvus_health_url
from shared.infra.vector_store import VectorDocument

vector_store_module = importlib.import_module("agents.capabilities.requirements.db.vector_store")
...
class TestVectorStoreGetStats:
    ...
    async def test_get_stats_sanitizes_uri(self):
        ...
        with patch("agents.capabilities.requirements.db.vector_store.settings") as mock_settings:
...
class TestVectorStoreFindSimilar:
    ...
    async def test_find_similar_uses_get_by_ids(self):
        ...
        with patch("agents.capabilities.requirements.db.vector_store.shared_embedder") as mock_emb:
...
class TestVectorStoreSearchSanitization:
    ...
    async def test_search_logs_query_length_not_query_text(self):
        ...
        with (
            patch("agents.capabilities.requirements.db.vector_store.shared_embedder") as mock_emb,
            patch("agents.capabilities.requirements.db.vector_store.logger") as mock_logger,
        ):
            mock_emb.embed.return_value = [0.1]
