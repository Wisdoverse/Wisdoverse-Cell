...
def generator(mock_bitable, mock_op):
    from agents.capabilities.analysis.core.weekly_report import WeeklyReportGenerator
...
async def test_generate_empty(generator, mock_bitable):
    ...
    with patch("agents.capabilities.analysis.core.weekly_report.settings") as mock_settings:
...
async def test_push_to_chat_success(generator):
    """成功推送周报到飞书群应返回 True"""
    with patch("agents.capabilities.analysis.core.weekly_report.settings") as mock_settings, \
         patch("agents.capabilities.analysis.core.weekly_report.get_feishu_client") as mock_get_client:
