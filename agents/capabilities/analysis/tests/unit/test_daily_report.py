...
def generator(mock_bitable, mock_op):
    from agents.capabilities.analysis.core.daily_report import DailyReportGenerator
...
async def test_generate_empty(generator, mock_bitable):
    ...
    with patch("agents.capabilities.analysis.core.daily_report.settings") as mock_settings:
...
async def test_push_to_chat_success(generator):
    """推送日报到飞书群应成功"""
    with patch("agents.capabilities.analysis.core.daily_report.settings") as mock_settings, \
         patch("agents.capabilities.analysis.core.daily_report.get_feishu_client") as mock_get_client:
