...
class UserInteractionOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending user-interaction gateway integration events."""

    name = "user-interaction-outbox-dispatcher"
    ...
    async def startup(self, runtime) -> None:
        self._task = asyncio.create_task(
            ...
            name="user_interaction_outbox_dispatcher",
        )
    ...
    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_user_interaction_events", None)
