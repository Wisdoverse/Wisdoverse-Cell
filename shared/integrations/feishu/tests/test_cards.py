"""Tests for feishu card builder and requirement card templates."""
...
class TestCardBuilder:
    ...
    def test_set_header__without_subtitle__no_subtitle_field(self):
        card = (
            CardBuilder()
            .set_header("Title", template="green")
            .build()
        )
    ...
    def test_add_button__appends_to_existing_action_group(self):
        ...
        # Both buttons should be in a single action element
        ...
        assert action_el["actions"][1]["text"]["content"] == "B"
    ...
    def test_add_button__creates_new_action_group_when_none_exists(self):
        ...
        # First element is div (text), second is a new action group
        assert card["elements"][0]["tag"] == "div"
        assert card["elements"][1]["tag"] == "action"
        assert len(card["elements"][1]["actions"]) == 1
    ...
    def test_build__with_header__includes_header_and_config(self):
        card = CardBuilder().set_header("H").build()

        assert "header" in card
        assert card["header"]["title"]["content"] == "H"
    ...
    def test_build_message__wraps_with_msg_type(self):
        ...
        assert len(msg["card"]["elements"]) == 1
    ...
    def test_add_note__adds_note_element(self):
        card = CardBuilder().add_note("备注文本").build()

        note = card["elements"][0]
        assert note["tag"] == "note"
        assert len(note["elements"]) == 1
        assert note["elements"][0] == {"tag": "plain_text", "content": "备注文本"}


# ---------------------------------------------------------------------------
# TestRequirementCards
# ---------------------------------------------------------------------------
