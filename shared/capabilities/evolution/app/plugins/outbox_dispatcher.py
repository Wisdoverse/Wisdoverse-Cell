"""EvolutionOutboxDispatcherPlugin — background evolution outbox retry loop."""
...
logger = get_logger("plugin.evolution-outbox-dispatcher")

_RUNTIME_LABEL = "evolution-module"


class EvolutionOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending evolution integration events."""

    name = "evolution-outbox-dispatcher"
    ...
    async def startup(self, runtime) -> None:
        self._task = asyncio.create_task(
            ...
            name="evolution_outbox_dispatcher",
        )
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("evolution_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_evolution_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("evolution_outbox_dispatcher_missing")
            return
