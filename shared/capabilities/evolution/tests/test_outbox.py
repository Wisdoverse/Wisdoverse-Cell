...
from shared.capabilities.evolution.service.agent import EvolutionModule
from shared.evolution.db.repository import EvolutionEventOutboxRepository
from shared.evolution.db.tables import EvolutionEventOutbox
...
def _outbox_row(**overrides):
    ...
    return MagicMock(spec=EvolutionEventOutbox, **defaults)


class FakeEvolutionOutboxStore:
    def __init__(self):
        self.add = AsyncMock()
...
@pytest.mark.asyncio
async def test_evolution_outbox_repository_add_preserves_event_contract():
    session = MagicMock()
    session.flush = AsyncMock()
    repo = EvolutionEventOutboxRepository(session)
    event = Event.create(
        event_type=EventTypes.EVOLUTION_SKILL_PROPOSED,
        source_agent="evolution-module",
        payload={"operation": "add_skill", "target_agent": "pjm-agent"},
    ...
    assert row.event_type == EventTypes.EVOLUTION_SKILL_PROPOSED
    assert row.source_agent == "evolution-module"
    assert row.payload["operation"] == "add_skill"
    assert row.trace_id == "trace-evolution"
...
async def test_publish_event_via_outbox_stages_before_publish():
    ...
    outbox_store = FakeEvolutionOutboxStore()
    ...
    event = Event.create(
        event_type=EventTypes.EVOLUTION_PATTERN_PROPOSED,
        source_agent="evolution-module",
        payload={"pattern_id": "pat_001"},
    )
...
@pytest.mark.asyncio
async def test_publish_pending_evolution_events_marks_failure_and_continues():
    db = _db_manager_with_session()
    bus = MagicMock()
    bus.connect = AsyncMock()
    bus.publish = AsyncMock(return_value=True)
    ...
    result = await agent.publish_pending_evolution_events(limit=2)
