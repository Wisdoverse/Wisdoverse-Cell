"""Application use cases for Evolution outbox delivery."""
...
logger = get_logger("evolution_module.outbox_delivery")


class EvolutionOutboxEventBusPort(Protocol):
...
class EvolutionOutboxEventPublisherPort(Protocol):
...
class EvolutionOutboxDeliveryUseCase:
    """Deliver Evolution integration events through the durable outbox."""

    def __init__(
        self,
        *,
        outbox_store: EvolutionEventOutboxStore,
        event_bus: EvolutionOutboxEventBusPort,
        event_publisher: EvolutionOutboxEventPublisherPort,
    ) -> None:
    ...
    async def publish_pending_events(self, limit: int = 100) -> dict[str, int]:
        ...
        logger.info(
            "evolution_outbox_dispatch_completed",
            total=len(rows),
            published=published,
            failed=failed,
        )
    ...
    async def publish_staged_event(self, event: Event) -> bool:
        ...
        try:
            ...
            logger.error(
                "evolution_outbox_publish_failed",
    ...
    async def mark_event_published(self, event: Event) -> None:
        ...
        try:
            ...
            logger.warning(
                "evolution_outbox_mark_published_failed",
                event_id=event.event_id,
                event_type=event.event_type,
                error=str(exc),
            )
    ...
    async def mark_event_failed(self, event: Event, error: Exception) -> None:
        ...
        try:
            ...
            logger.warning(
                "evolution_outbox_mark_failed_failed",
                event_id=event.event_id,
                event_type=event.event_type,
