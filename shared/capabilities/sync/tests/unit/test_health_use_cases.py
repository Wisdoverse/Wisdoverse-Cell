...
from shared.capabilities.sync.core.health_use_cases import SyncHealthUseCase
...
async def test_health_check_reports_database_ready() -> None:
    use_case = SyncHealthUseCase(health_store=FakeHealthStore(True))
...
async def test_health_check_reports_database_not_ready() -> None:
    use_case = SyncHealthUseCase(health_store=FakeHealthStore(False))
