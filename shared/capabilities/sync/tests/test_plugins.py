...
class TestSyncOutboxDispatcherPlugin:
    @pytest.mark.asyncio
    async def test_dispatch_once_calls_agent_outbox_dispatcher(self):
        from shared.capabilities.sync.app.plugins.outbox_dispatcher import (
            SyncOutboxDispatcherPlugin,
        )

        plugin = SyncOutboxDispatcherPlugin(interval=100, batch_size=7)
        runtime = MagicMock()
        runtime.agent.publish_pending_sync_events = AsyncMock(
            return_value={"total": 1, "published": 1, "failed": 0}
        )

        await plugin._dispatch_once(runtime)

        runtime.agent.publish_pending_sync_events.assert_awaited_once_with(limit=7)
    ...
    async def test_health_ok_when_running(self):
        ...
        plugin = SyncOutboxDispatcherPlugin(interval=100)
