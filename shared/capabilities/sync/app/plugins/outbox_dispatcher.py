"""SyncOutboxDispatcherPlugin — background Sync outbox retry loop."""
...
logger = get_logger("plugin.sync-outbox-dispatcher")

_RUNTIME_LABEL = "sync-module"


class SyncOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending Sync integration events."""

    name = "sync-outbox-dispatcher"
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("sync_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_sync_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("sync_outbox_dispatcher_missing")
            return
