...
from shared.capabilities.analysis.core.outbox_delivery_use_cases import (
    AnalysisOutboxDeliveryUseCase,
)
...
def test_event_from_outbox_preserves_event_contract() -> None:
    ...
    assert event.event_id == "evt_analysis_01"
    assert event.event_type == EventTypes.ANALYSIS_RISK_DETECTED
    assert event.source_agent == "analysis-module"
