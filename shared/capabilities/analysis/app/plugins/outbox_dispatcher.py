"""AnalysisOutboxDispatcherPlugin — background analysis outbox retry loop."""
...
logger = get_logger("plugin.analysis-outbox-dispatcher")

_RUNTIME_LABEL = "analysis-module"


class AnalysisOutboxDispatcherPlugin(RuntimePlugin):
    """Periodically retry pending analysis integration events."""

    name = "analysis-outbox-dispatcher"
    ...
    async def startup(self, runtime) -> None:
        self._task = asyncio.create_task(
            ...
            name="analysis_outbox_dispatcher",
        )
    ...
    async def _dispatcher_loop(self, runtime) -> None:
        while True:
            try:
                ...
                logger.error("analysis_outbox_dispatch_error", error=error_type)
            await asyncio.sleep(self._interval)

    async def _dispatch_once(self, runtime) -> None:
        dispatcher = getattr(runtime.agent, "publish_pending_analysis_events", None)
        if dispatcher is None:
            self._last_error = "dispatcher_not_available"
            logger.warning("analysis_outbox_dispatcher_missing")
            return
