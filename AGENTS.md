...
```
agents/
  chat_agent/                 # Claude Tool Calling + PM 助手
  coordinator/                # CEO 全局编排引擎 (跨 Agent 决策)
  pjm_agent/                   # 任务拆解/审批/预警/报表
  sync_agent/                 # OP ↔ 飞书双向同步
  analysis_agent/             # 风险检测/数据分析
  requirement_manager/        # 需求提取/确认/PRD 生成
  evolution_agent/            # 自进化引擎 (全局分析/建议模式)
  qa_agent/                   # QA 验收 Agent (自动化代码质量验收)
  dev_agent/                  # 自动化开发 Agent (AgentForge 集成)
